...
try:
    # If 'recsys' is a package (preferred on Cloud)
    from recsys.recsys_core import (
        bootstrap_data, search_movies, movie_card, similar_items,
    )
except ModuleNotFoundError:
    try:
        # If the module is flat beside this file (local dev)
        from recsys_core import (
            bootstrap_data, search_movies, movie_card, similar_items,
        )
    except ModuleNotFoundError:
        # Last resort: add repo root to sys.path then import
...
def main():
    ...
    if sel:
        ...
        with c2:
            ...
            k_sims = st.slider("How many suggestions do you want?", 5, 20, 12, 1, key="k_sims")
