...
def add_title_features(movies: pd.DataFrame) -> pd.DataFrame:
    ...
    out = movies.copy(); out["clean_title"] = list(base); out["year"] = list(years)
    return out
    ...
    for start in range(0, n_items, block):
        ...
        for j in range(cs.shape[1]):
            ...
            if col.nnz > k:
                ...
                idx = col.row[top]; val = col.data[top]
                order = np.argsort(-val); idx = idx[order]; val = val[order]
            else:
                idx = col.row; val = col.data
                order = np.argsort(-val); idx = idx[order]; val = val[order]
            indices.extend(idx.tolist()); data.extend(val.tolist())
            indptr.append(indptr[-1] + len(idx))
    return sparse.csr_matrix(
        (np.array(data,np.float32), np.array(indices,np.int32), np.array(indptr,np.int32)),
        shape=(n_items,n_items)
    )

def _save_sparse(path: str, mat: sparse.csr_matrix, meta: dict) -> None:
    _ensure_dir(os.path.dirname(path))
    np.savez_compressed(path, data=mat.data, indices=mat.indices, indptr=mat.indptr,
                        shape=mat.shape, meta=json.dumps(meta))
...
def _filter_popular_items(ratings: pd.DataFrame, min_ratings: int, max_items: int) -> set[int]:
...
def bootstrap_data(
    ...
    if os.path.exists(cache):
        try:
            ...
            ok = (
                meta.get("shape") == list(R.shape) and
...
def search_movies(bundle: DataBundle, query: str, limit: int = 25) -> pd.DataFrame:
    ...
    m = _YEAR_NUMBER.search(q_raw); q_year = int(m.group(1)) if m else None
